...
function initEventHandlers()
{
    ...
    canvas.addEventListener("mousemove", ghostedShape);
    canvas.addEventListener("mousedown", mouseDown);
    canvas.addEventListener("mouseup", mouseUp);
    ghostInterval = setInterval(renderGhostShape, 60);
...
function ghostedShape(event) {
    ...
    ;
    if (patternsList.selectedIndex == 24 && mouseIsDown == 1) {
    ...
    if (patternsList.selectedIndex == 25 && mouseIsDown == 1) {
...
function respondToMouseClick(event)
{
    ...
    if (patternsList.selectedIndex < 24) {
        ...
        for (var i = 0; i < pixels.length; i += 2)
        {
            ...
            var y = row * cellLength;
            canvas2D.fillRect(x, y, cellLength, cellLength);
            setGridCell(brightGrid, row, col, LIVE_CELL);
...
function renderBrighter() {
    ...
    for (var i = 0; i <= gridHeight; i++)
    {
        for (var j = 0; j < gridWidth; j++)
        {
            var cell = getGridCell(brightGrid, i, j);
            if (cell === LIVE_CELL)
            {
                var x = j * cellLength;
                var y = i * cellLength;
